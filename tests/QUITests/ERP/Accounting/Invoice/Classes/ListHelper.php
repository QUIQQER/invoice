...
class ListHelper
{
    ...
    public static function outputList(QUI\ERP\Accounting\ArticleList $List)
    {
        ...
        foreach ($data['articles'] as $article) {
            ...
            writePhpUnitMessage('    Calc Netto Sum: ' . $Currency->format($article['calculated_nettoSum'], $Locale));
            writePhpUnitMessage('    Calc Price: ' . $Currency->format($article['calculated_price'], $Locale));
            writePhpUnitMessage('    Calc Sum: ' . $Currency->format($article['calculated_sum'], $Locale));

            writePhpUnitMessage(
                '    -> Vat ' . $article['calculated_vatArray']['vat'] . '% : ' .
                $article['calculated_vatArray']['text'] . ' ' .
                $Currency->format($article['calculated_vatArray']['sum'], $Locale)
            );
